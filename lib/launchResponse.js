...
const launchResponse = ({event, lti, aws, eventToRequest, readSchoolConfig, findIliosUser, fetch, createJWT, Redis}) => {
    ...
    readSchoolConfig(consumerKey, aws).then(config => {
      ...
      provider.valid_request(request, (error, isValid) => {
        if (isValid) {
          ...
          findIliosUser({fetch, createJWT, config, searchString, aws, Redis}).then(userId => {
